 ...
 * Can be called from the worker process to run in-process,
 * or the file can be run standalone via `tsx src/server/websocket.ts`.
 */
...
// If run directly as standalone, auto-start
const isDirectRun = require.main === module || process.argv[1]?.includes("websocket");
if (isDirectRun) {
  initWebSocketServer();
}
