...
async function fetchJockeyProfile(jockeyId: string): Promise<JockeyProfile> {
  const res = await fetch(`/api/jockeys/${jockeyId}`);
  if (!res.ok) throw new Error("Failed to fetch jockey profile");
  return res.json();
}
...
export default function JockeyProfilePage() {
  ...
  const {
    ...
    queryFn: () => fetchJockeyProfile(jockeyId),
  });
      ...
      <div className="flex min-h-screen items-center justify-center">
        <Card className="w-full max-w-md">
          <CardHeader>
            ...
            <CardDescription>
              Failed to load jockey profile. Please try again.
            </CardDescription>
          </CardHeader>
          <CardContent>
  ...
  const venueChartData = jockey.venuePerformance.map((venue) => ({
  ...
  return (
    <div className="container mx-auto space-y-6 py-8">
          ...
          <div className="flex items-start justify-between">
            <div>
              <div className="flex items-center gap-3">
                <CardTitle className="text-3xl">{jockey.name}</CardTitle>
              ...
              <CardDescription className="mt-2 text-lg">
                {jockey.overallStats.wins}-{jockey.overallStats.places}-
                {jockey.overallStats.losses} record •{" "}
                <span className="font-semibold text-green-600">
                  {jockey.overallStats.winRate}% win rate
                </span>
              </CardDescription>
            </div>
          </div>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        <Card>
          <CardHeader className="pb-3">
            <CardDescription>Total Horses</CardDescription>
            <CardTitle className="text-3xl">
              {jockey.overallStats.totalRunners}
            </CardTitle>
          </CardHeader>
        </Card>
        <Card>
        ...
        <Card>
          <CardHeader className="pb-3">
            ...
            <CardTitle className="text-3xl">
              {jockey.overallStats.totalRaces}
            </CardTitle>
          </CardHeader>
        </Card>
      </div>
      ...
      {jockey.venuePerformance.length > 0 && (
        <Card>
          <CardHeader>
          ...
          <CardContent className="space-y-6">
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {jockey.venuePerformance.map((venue) => (
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            ...
            Horses ({jockey.runners.length})
          </CardTitle>
          <CardDescription>
            All horses ridden by {jockey.name}
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="overflow-x-auto">
