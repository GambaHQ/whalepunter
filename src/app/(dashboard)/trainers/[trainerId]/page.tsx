...
async function fetchTrainerProfile(trainerId: string): Promise<TrainerProfile> {
...
export default function TrainerProfilePage() {
  ...
  const {
    ...
    queryFn: () => fetchTrainerProfile(trainerId),
  });
      ...
      <div className="flex min-h-screen items-center justify-center">
        <Card className="w-full max-w-md">
          <CardHeader>
            ...
            <CardDescription>
              Failed to load trainer profile. Please try again.
            </CardDescription>
          </CardHeader>
          <CardContent>
  ...
  const venueChartData = trainer.venuePerformance.map((venue) => ({
          ...
          <div className="flex items-start justify-between">
            <div>
              ...
              <CardDescription className="mt-2 text-lg">
                {trainer.overallStats.wins}-{trainer.overallStats.places}-
                {trainer.overallStats.losses} record •{" "}
                <span className="font-semibold text-green-600">
                  {trainer.overallStats.winRate}% win rate
                </span>
              </CardDescription>
            </div>
          </div>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        <Card>
          <CardHeader className="pb-3">
            <CardDescription>Total Runners</CardDescription>
            <CardTitle className="text-3xl">
              {trainer.overallStats.totalRunners}
            </CardTitle>
          </CardHeader>
        </Card>
        <Card>
        ...
        <Card>
          <CardHeader className="pb-3">
            ...
            <CardTitle className="text-3xl text-blue-600">
              {trainer.overallStats.placeRate}%
            </CardTitle>
          </CardHeader>
        </Card>
        <Card>
          <CardHeader className="pb-3">
            ...
            <CardTitle className="text-3xl">
              {trainer.overallStats.totalRaces}
            </CardTitle>
          </CardHeader>
        </Card>
      </div>
        ...
        <CardContent className="space-y-6">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {trainer.venuePerformance.map((venue) => (
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            ...
            Runners ({trainer.runners.length})
          </CardTitle>
          <CardDescription>
            All runners trained by {trainer.name}
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="overflow-x-auto">
